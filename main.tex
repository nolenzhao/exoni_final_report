...
% STUB_NOLEN: Close the loop on every firmware constraint from Table~\ref{tab:fw_constraints}.
% Add a table like the one below (adapt tab:results style), then write 1-2 paragraphs.
%
% Suggested table structure:
%
% \begin{table}[H]
% \centering
% \caption{Firmware Constraint Verification}
% \label{tab:fw_results}
% \begin{tabular}{llll}
% \toprule
% Constraint              & Target          & Achieved        & Evidence \\
% \midrule
% FOC inner loop rate     & 1 kHz           & 1 kHz           & vTaskDelayUntil, TIM1 ARR=399 @ 16 MHz \\
% Outer impedance loop    & 200 Hz          & 200 Hz          & xTaskNotifyGive every 5th tick \\
% PWM switching freq      & 20 kHz          & 20 kHz          & TIM1 center-aligned, ARR=399 \\
% Current sense res.      & 12-bit ADC      & 12-bit          & ADC3/ADC4 on STM32G474 \\
% Encoder resolution      & $\leq$0.022\textdegree/count & 0.022\textdegree/count & AS5048A, 16384 counts/rev \\
% Phase overcurrent trip  & 15 A            & 15 A            & FOC\_ILIMIT in foc.h \\
% Telemetry latency       & $\leq$5 ms      & 5 ms frame period & 75-byte frame @ 921600 baud \\
% \bottomrule
% \end{tabular}
% \end{table}
%
% Then add 1-2 paragraphs on CLOSED-LOOP FOC VALIDATION:
% - In closed-loop mode at Iq_ref = 0.5 A: i_d converged to near zero, i_q tracked
%   reference with low steady-state error (confirm from telemetry CSV logs)
% - DRV8323 nFAULT pin remained high throughout -- no gate driver faults triggered
%
% Then add a paragraph on GAIT CLASSIFIER + IMPEDANCE VALIDATION:
% - Reference the March 20, 2026 test data (imu_log_20260320_*.csv)
% - The classifier output (gait_phase column) and control signals (ctrl_tau, ctrl_Iq)
%   were logged at 200 Hz via the UDP telemetry pipeline
% - TODO: generate a time-series plot (1 figure) showing gait_phase, ctrl_tau, ctrl_Iq
%   over ~15 s of walking -- this is your key result figure for the controls section
% - Comment on whether the impedance limits (+/-18 Nm torque, +/-10 A current) were
%   ever saturated during the test and what that indicates
