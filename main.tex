...
% STUB_NOLEN: Write ~half a page. Must stand alone for a non-expert reader.
% Four elements required by the rubric:
%
% (1) PROBLEM
%     Exoni targets mobility-limited users for outdoor use (hiking/running).
%     Core challenge: high torque + low backdrive + passive thermal safety + minimal mass
%     in one device -- no off-the-shelf motor or control system satisfies all four.
%
% (2) FINAL DESIGN (one sentence per subsystem)
%     Mechanical: custom 8110-frame BLDC with Halbach rotor, 8.4:1 planetary gearbox
%     Electrical: three custom PCBAs (primary, encoder, IMU) + 8S1P Li-ion battery pack
%     Firmware/Control: STM32G474 FreeRTOS dual-task -- 1 kHz FOC inner loop +
%       200 Hz gait-adaptive impedance control driven by an on-device random forest
%       gait classifier (3 phases: PRE_SWING, STANCE, SWING)
%
% (3) KEY RESULTS (state each against its target -- mirror Table tab:results)
%     Gearbox output torque: 20 Nm (target 17.6 Nm) CHECK
%     Backdrive torque: 0.26 Nm (target <1 Nm) CHECK
%     Surface temperature: 35 degC (target <=43 degC) CHECK
%     Motor weight: 582 g (target 600 g) CHECK
%     Full system weight: 1.5 kg (target 1.5 kg) CHECK
%     Firmware: all 7 constraints met; closed-loop FOC confirmed (i_d->0, i_q tracks ref)
%     Gait classifier: running at 200 Hz on-device, phase output confirmed in test logs
%
% (4) CONCLUSIONS
%     Simulation-driven design was essential -- none of the three key decisions
%     (stator frame, rotor config, battery config) could be made correctly by intuition.
%     Platform is validated at the subsystem level and ready for gait-in-the-loop testing.
\newpage
...
The 200~Hz ControlTask implements a two-step virtual impedance law. First, a spring-damper equation computes a target joint torque from the measured knee state:
\begin{equation}
...
different phases. The classifier output selects which row of this table is active at each 200~Hz tick, and those parameters are passed directly into the impedance law and torque-to-current conversion described above.
...
\begin{equation}
    ...
    \label{eq:baud_rate}
\end{equation}
This is one-way, so the round-trip cost before any inference begins is at minimum $\sim$0.6~ms. In addition, the task overhead of the ESP32 servicing Wi-Fi communication to external listeners can cause temporary spikes in latency, making the effective inference rate inconsistent. The random forest was trained on features extracted at a fixed 200~Hz rate; running inference at a faster, non-uniform rate would introduce a resolution mismatch between training and deployment, artificially degrading classifier accuracy and causing the impedance controller to perform unpredictably. All gait classification and impedance computation therefore executes on the STM32G474, 
keeping the full pipeline at a steady 200~Hz with the ESP32 used only as a passive wireless telemetry bridge for analysis and debugging.
