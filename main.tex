...
Exoni is a powered knee exoskeleton designed for outdoor recreational use by mobility-limited individuals. The core engineering challenge is that a wearable knee actuator must simultaneously deliver high output torque, remain backdrivable when unpowered, stay cool enougoh not to cause discomfort, and fit within a strict mass budget to keep the goal of aiding the user. No off-the-shelf motor or control system satisfied all four requirements, which drove 
...
with particular emphasis on the conflicting demands of high torque output, low backdrive resistance, passive thermal safety, minimal mass, and responsivle controls.
...
The 200Hz control cycle constraint ensures that logged data is coherent with the control state and remains responsive enough for each phase of the gait cycle. It sits inbetween
...
is the coress-referencing mentioned earlier. The MOCAP data acts at the ground truth.
...
This is critical because consecutive 5~ms frames within one trial are temporally correlated. If using naive random split this would allow the model to memorize trial-specific patterns from timestampts and report artificially high accuracy. 
Second, K-fold cross-validation is used, keeping trial groups intact across folds for the reason above. 
Third, the classifier is trained to compensate for unequal phase durations inherent in stair gait. 
Given stance occupies a larger fraction of each cycle than pre-swing or swing, the model is reweighted to not be biased towards predicting the majority class. 
The features are also normalized to have similar dynamic range among them for preprartiong of quantization where values are scaled.
Finally quantization of the weights occurs to convert from the default 32-bit floating-point weights 
to 16-bit integers. This ensures the model size is able to fit inside of the flash memory of the STM32. 
which maps normalized floats to \texttt{int16} values. This is done by simply multiplying the floats by some static scalar. 
The trained forest is then exported to a C header via \verb|emlearn|, producing a fully inlined header file, which is compiled directly into the firmware flash. 
This allows these values to be loaded onto the stack, bypassing the need for heap allocation which can have unpredictable timing 
due to the memory allocation/deallocation.

\vspace{10px}
Following these steps to develop the model, the predcition accuracy is about 87\% for all three gait phases.
...
automatically clean and label this mocap data for cross-reference with it's corresponding Exoni on-device data.
...
% STUB_NOLEN: Full 45-feature table. Columns: index, name, type (raw/derived/windowed),
% units, quantisation scale factor (from GAIT_QUANT_SCALES[] in feature_scales.h).
% Example rows:
%   0  enc_deg         raw         deg      91.027
%   1  imu1_ax         raw         g      8197.488
%   ...
%   12 imu2_gz         raw         dps    ...
%   13 enc_vel         derived     deg/s  ...
%   19 rel_gyro_z      derived     dps    ...
%   20 enc_deg_mean    windowed    deg    ...
%   21 enc_deg_std     windowed    deg    ...
%   ...
%   44 imu2_gz_std     windowed    dps    ...
% Scale factors can be read directly from feature_scales.h in the firmware repo.

\subsection{Impedance Control Torque-to-Current Derivation}
% STUB_NOLEN: Show the full signal chain:
%   tau = K*(theta_ref - theta) + B*(omega_ref - omega)      [Nm]
%   tau_backdrive = 0.26 Nm  if |omega| > 5 deg/s, else 0   [Nm]
%   I_q = (tau + tau_backdrive) / (K_t * G_ratio)           [A]
%         K_t = 0.97 Nm/A (measured, 2nd motor iteration)
%         G_ratio = 8.4
%   Clamps: tau in [-18, +18] Nm;  I_q in [-10, +10] A
% State the maximum output torque at the 10 A soft limit:
%   tau_max = 10 A * 0.97 Nm/A * 8.4 = 81.5 Nm >> 18 Nm saturation limit
